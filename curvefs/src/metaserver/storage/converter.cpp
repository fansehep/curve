...
std::string NameGenerator::Format(KEY_TYPE type, uint32_t partitionId) {
    ...
    return absl::StrCat(type, kDelimiter,
        absl::string_view(buf, sizeof(uint32_t)));
}
...
inline std::string Prefix4ChunkIndexS3ChunkInfoList::SerializeToString() const {
    return absl::StrCat(keyType_, ":", fsId, ":", inodeId, ":",
        chunkIndex, ":");
}
...
inline std::string Prefix4InodeS3ChunkInfoList::SerializeToString() const {
    return absl::StrCat(keyType_, ":", fsId, ":", inodeId, ":");
}
...
inline std::string Prefix4AllS3ChunkInfoList::SerializeToString() const {
...
inline std::string Prefix4AllDentry::SerializeToString() const {
    return absl::StrCat(keyType_, ":");
}
